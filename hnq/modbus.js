...
Modbus.prototype.f0x01 = function(slave, addr, cnt, done) { //Function reading coils(0x01)
	var sbuff = makeRequest(slave, 0x01, addr, cnt),
			rbuff = Buffer.alloc((cnt % 8 ? 6 : 5) + Math.floor(cnt / 8));
	this._sndrcv(sbuff, rbuff, function(err, buff) {
		done(err, err || extractBits(buff, cnt));
	});
};

Modbus.prototype.f0x02 = function(slave, addr, cnt, done) { //Function reading discrete inputs (0x02)
...
Modbus.prototype.f0x03 = function(slave, addr, cnt, done) { //Function reading holding register 16bit (0x03)
	var sbuff = makeRequest(slave, 0x03, addr, cnt),
			rbuff = Buffer.alloc(5 + cnt * 2);
	this._sndrcv(sbuff, rbuff, function(err, buff) {
		done(err, err || extractWords(buff, cnt));
	});
};
Modbus.prototype.f0x0332 = function(slave, addr, cnt, done) { //Function reading holding register 32bit(0x03)
    var sbuff = makeRequest(slave, 0x03, addr, cnt),
        rbuff = Buffer.alloc(5 + cnt * 4);
    this._sndrcv(sbuff, rbuff, function(err, buff) {
        done(err, err || extractWords32(buff, cnt));
    });
};

Modbus.prototype.f0x04 = function(slave, addr, cnt, done) { //Function reading input register 16bit (0x04)
...
Modbus.prototype.f0x0432 = function(slave, addr, cnt, done) { //Function reading input register 32bit (0x04)
    var sbuff = makeRequest(slave, 0x04, addr, cnt),
        rbuff = Buffer.alloc(5 + cnt * 2);
    this._sndrcv(sbuff, rbuff, function(err, buff) {
        done(err, err || extractWords32(buff, cnt));
...
Modbus.prototype.writeCoil = function(slave, addr, val, done) { //Function writing single coil (0x05)
	var sbuff = makeRequest(slave, 0x05, addr, val ? 0xff00 : 0x0000),
			rbuff = slave ? Buffer.alloc(8) : null;
...
function extractWords32(buff, cnt) {
    var data = [];
    for (var i = 0; i < cnt; ++i)
        data.push(buff.readFloatBE(3 + i * 2).toFixed(2));
    return data;
}
